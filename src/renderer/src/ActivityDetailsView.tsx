    ...
    <section className="activity-main">
      ...
      {mode === 'details' && <>
        <section className="activity-timeline-card">
          <div className="activity-section-head"><div><p className="eyebrow">RECORDED RANGE</p><h2>{details?.rangeStart ? `${clock(details.rangeStart)}–${clock(details.rangeEnd ?? details.rangeStart)}` : '当天记录区间'}</h2></div><div title={`含 ${duration(details?.softIdleSeconds ?? 0)} 低交互推定时间`}><strong>{duration(details?.activeSeconds ?? 0)}</strong><small>推定工作投入 · {details?.idleThresholdMinutes ?? 5} 分钟内计入</small></div></div>
