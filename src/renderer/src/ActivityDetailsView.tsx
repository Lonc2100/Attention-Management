    ...
    <section className="activity-main">
      ...
      {mode === 'details' && <>
        <section className="activity-timeline-card">
          <div className="activity-section-head"><div><p className="eyebrow">RECORDED RANGE</p><h2>{details?.rangeStart ? `${clock(details.rangeStart)}–${clock(details.rangeEnd ?? details.rangeStart)}` : '当天记录区间'}</h2></div><div title={`含 ${duration(details?.softIdleSeconds ?? 0)} 低交互推定时间`}><strong>{duration(details?.activeSeconds ?? 0)}</strong><small>推定工作投入 · {details?.idleThresholdMinutes ?? 15} 分钟内计入</small></div></div>
...
function EvidenceDrawer({ entry, details, onClose, mutate }: {
  ...
  return <aside className="evidence-drawer"><button className="drawer-close" onClick={onClose}>×</button><p className="eyebrow">SELECTED EVIDENCE</p><h2>这 {duration(entry.seconds)} 在做什么？</h2><div className="evidence-card"><label>时间<strong>{clock(entry.start)}–{clock(entry.end)}</strong></label><label>应用<strong>{entry.app || '—'}</strong></label><label>窗口标题<strong>{entry.title || '—'}</strong></label><label>当前归类<strong>{entry.projectLabel}</strong></label><label>判断来源<strong>{entry.idleOverrideId ? '人工计入 · 原始记录为离开电脑' : SOURCE_LABEL[entry.attribution]}{entry.attribution === 'manual' ? ' · 最高优先级' : ''}</strong></label></div>{entry.attribution === 'afk' && <div className="correction-form idle-correction"><p>ActivityWatch 只确认这段时间没有键鼠输入，不能确认你是否真的离开。若你仍在阅读、思考或观看素材，可以把这段计入工作。</p><button className="primary full" data-testid="idle-override-add" onClick={() => void mutate(() => window.timeEfficiency.addIdleOverride({ date: details?.date ?? '', start: entry.start, end: entry.end }), '这段时间已人工计入工作')}>这段时间其实仍在工作</button></div>}{entry.idleOverrideId && <div className="correction-form idle-correction"><p>这段时间由你人工计入工作，ActivityWatch 的原始 AFK 记录没有被改写。</p><button className="text-button undo" data-testid="idle-override-remove" onClick={() => void mutate(() => window.timeEfficiency.removeIdleOverride({ date: details?.date ?? '', overrideId: entry.idleOverrideId ?? '' }), '已恢复为离开电脑')}>恢复为离开电脑</button></div>}{entry.correctable && entry.attribution !== 'afk' && !entry.idleOverrideId && <div className="correction-form"><label><span>归入项目</span><select value={projectKey} onChange={(event) => setProjectKey(event.target.value)}><option value="">请选择项目</option>{details?.projectOptions.map((project) => <option key={project.key} value={project.key}>{project.label}</option>)}<option value="__new__">＋ 新建项目</option></select></label>{projectKey === '__new__' && <label><span>新项目名称</span><input autoFocus maxLength={60} value={newProject} onChange={(event) => setNewProject(event.target.value)} placeholder="例如：自媒体创作" /></label>}<label className="learn-check"><input type="checkbox" checked={learnRule} onChange={(event) => setLearnRule(event.target.checked)} /><span>以后同类活动自动归类<small>新规则只影响保存之后，不改写过去。</small></span></label>{learnRule && <div className="rule-editor"><select value={titleMatch} onChange={(event) => setTitleMatch(event.target.value as 'contains' | 'exact')}><option value="contains">标题包含</option><option value="exact">标题完全等于</option></select><input value={pattern} onChange={(event) => setPattern(event.target.value)} />{!meaningful && <small>标题条件至少 4 个字符；本次仍可只保存人工纠错。</small>}</div>}<button className="primary full" disabled={!projectKey || (projectKey === '__new__' && !newProject.trim())} onClick={() => void save()}>保存纠错</button>{entry.overrideId && <button className="text-button undo" onClick={() => void mutate(() => window.timeEfficiency.removeActivityCorrection({ date: details?.date ?? '', overrideId: entry.overrideId ?? '' }), '已撤销人工纠错')}>撤销这次纠错</button>}</div>}</aside>
}
